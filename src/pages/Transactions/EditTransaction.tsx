      ...
      try {
        const response = await fetch('https://localhost:44308/Transactions', {
          ...
          body: JSON.stringify({  "transactionId": TransactionId,
                                  "productId":  (document.getElementById("form_productId") as HTMLInputElement).value,
                                  ...
                                  "warehouseRackId": (document.getElementById("form_warehouseRackId") as HTMLInputElement).value,
      ...
      <div>
        ...
        <div className="flex flex-col justify-between mt-20 lg:ml-80 mr-5 mb-24 ml-20">
          <form className="flex max-w-md flex-col gap-4" onSubmit={handleSubmit}>
            <div className="font-semibold text-xl">Edit transaction </div>
            <div>
            ...
            <div>
              <div className="mb-2 block">
                <Label htmlFor="roles" value="Select warehouse rack" />
              </div>
              <Select id="form_warehouseRackId" required>
                {warehouseRacks.map((warehouseRack:any) =>
                <option>{warehouseRack.id}</option>)}
              </Select>
            </div>
            <div>
              <div className="mb-2 block">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button type="submit">Change transaction info</Button>
          </form>
          <div className="max-w-md mt-5">
            <Button color="failure" href="/Transactions">Back</Button>
          </div>
          <Alert color="success" className="mt-5 hidden" id="alert">
            <span className="font-medium text-lg">The information about transaction has been changed!</span>
          </Alert>
        </div>
        <FooterSite/>
      </div>
