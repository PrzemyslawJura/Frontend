...
function CreateWarehouseRack() {
  ...
  const { WarehouseRackId } = location.state;
  ...
  const handleSubmit = async (e:any) => {
    e.preventDefault();
    try {
      {console.log( (document.getElementById("form_warehouseSizeId") as HTMLInputElement).value) }
      const response = await fetch('https://localhost:44308/WarehouseRacks/WarehouseRack', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({  "id": WarehouseRackId,
      ...
      <div>
        ...
        <div className="flex flex-col justify-between mt-20 lg:ml-80 mr-5 mb-24 ml-20">
          <form className="flex max-w-md flex-col gap-4" onSubmit={handleSubmit}>
            <div className="font-semibold text-xl">Edit warehouse rack </div>
              <div>
              ...
              <Button type="submit">Edit warehouse rack</Button>
          </form>
          <div className="max-w-md mt-5">
            <Button color="failure" href="/WarehouseRacks">Back</Button>
          </div>
          <Alert color="success" className="mt-5 hidden" id="alert">
            <span className="font-medium text-lg">The information about warehouse rack has been changed!</span>
          </Alert>
        </div>
        <FooterSite/>
      </div>
