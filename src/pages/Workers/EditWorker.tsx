      ...
      try {
        const response = await fetch('https://localhost:44308/Administrator/Worker', {
          ...
          body: JSON.stringify({  "id": WorkerId,
                                  "firstName":  (document.getElementById("form_firstName") as HTMLInputElement).value,
      ...
      <div>
        ...
        <div className="flex flex-col justify-between mt-20 lg:ml-80 mr-5 mb-24 ml-20">
          <form className="flex max-w-md flex-col gap-4" onSubmit={handleSubmit}>
            <div className="font-semibold text-xl">Edit worker </div>
              <div>
              ...
              <Button type="submit">Change worker info</Button>
          </form>
          <div className="max-w-md mt-5">
            <Button color="failure" href="/Workers">Back</Button>
          </div>
          <Alert color="success" className="mt-5 hidden" id="alert">
            <span className="font-medium text-lg">The information about worker has been changed!</span>
          </Alert>
        </div>
        <FooterSite/>
      </div>
